...
public class WeightsComposite extends Composite {
	...
	public WeightsComposite(Composite parentComp) {
		...
		weightsLabel = new Label(comp, SWT.NONE);
	...
	public void resetComposite() {
		weightsLabel.setText("You need to solve the problem first.");
		FormData fdata = new FormData();
		fdata.top = new FormAttachment(0, 10);
		fdata.left = new FormAttachment(0, 20);
		weightsLabel.setLayoutData(fdata);
				

		fdata = new FormData();
		fdata.top = new FormAttachment(weightsLabel, 20);
		fdata.left = new FormAttachment(0, 20);
	...
	public void displaySolution(LDEAProblem ldeap) {
		...
		for(int i = 0; i < ldeap.getDMUNames().size(); i++) {
			...
			for(int j = 0; j < nbVar; j++) {
				tempArr.add(Double.toString(MathUtils.round(ldeap.getLdeapSolution().getWeights()[i][j],OSDEAParameters.getRoundingDecimals())));
			}
			data.add(tempArr);
		}
		...
		weightsLabel.setText("The problem weights are as follows: ");
